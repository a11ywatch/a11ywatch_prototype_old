...
import { signJwt, decodeJwt, verifyJwt } from "./utils";
import { EMAIL_ERROR, GENERAL_ERROR, CRAWLER_FINISHED } from "./strings";
...
export const Mutation = {
  ping(_, { jwt }, context) {
    ...
    const currentUser = decodeJwt(jwt);
    ...
    if (currentUser?.payload?.email) {
      ...
      return {
        jwt: verified?.email ? signJwt(verified.email) : "Invalid_token",
      };
    }
    return null;
  },
